...
#define CLOCKWORK_TUNABLES(TUNE, NO_TUNE)                         \
    ...
    NO_TUNE(probcut_min_depth, 5, 1, 20, 0.5, 0.002)              \
                                                                  \
    /* SEE Values */                                              \
    TUNE(quiesce_see_threshold, 30, -1000, 200, 20, 0.002)        \
    TUNE(movepicker_see_capthist_divisor, 53, 16, 192, 10, 0.002) \
    TUNE(see_pawn_val, 100, 50, 200, 8, 0.002)                    \
    TUNE(see_knight_val, 319, 150, 600, 23, 0.002)                \
    TUNE(see_bishop_val, 290, 150, 600, 23, 0.002)                \
    TUNE(see_rook_val, 561, 250, 1000, 38, 0.002)                 \
    TUNE(see_queen_val, 967, 450, 1800, 68, 0.002)                \
                                                                  \
    /* Stat Bonus */                                              \
    TUNE(stat_bonus_max, 1868, 948, 3792, 142, 0.002)             \
    TUNE(stat_bonus_quad, 4, 2, 8, .5, 0.002)                     \
    TUNE(stat_bonus_lin, 96, 60, 240, 9, 0.002)                  \
    TUNE(stat_bonus_sub, 142, 60, 240, 9, 0.002)                  \
                                                                  \
    /* Stat Malus */                                              \
    TUNE(stat_malus_max, 1203, 948, 3792, 142, 0.002)             \
    TUNE(stat_malus_quad, 3, 2, 8, .5, 0.002)                     \
    TUNE(stat_malus_lin, 127, 60, 240, 9, 0.002)                  \
    TUNE(stat_malus_sub, 117, 60, 240, 9, 0.002)                  \
                                                                  \
    /* Search Params */                                           \
    TUNE(asp_window_delta, 26, 25, 100, 4, 0.002)                 \
    NO_TUNE(razor_depth, 7, 1, 20, 0.5, 0.002)                    \
    TUNE(razor_margin, 680, 353, 1414, 53, 0.002)                 \
    NO_TUNE(lmp_depth_mult, 3, 1, 20, 0.5, 0.002)                 \
                                                                  \
    /* Futility Pruning */                                        \
    TUNE(ffp_margin_base, 427, 250, 1000, 38, 0.002)              \
    TUNE(ffp_margin_mult, 91, 50, 200, 8, 0.002)                  \
    TUNE(ffp_hist_div, 21, 16, 64, 3, 0.002)                      \
    NO_TUNE(ffp_depth, 8, 1, 20, 0.5, 0.002)                      \
                                                                  \
    /* Quiet History Pruning */                                   \
    NO_TUNE(qhp_depth, 4, 1, 20, 0.5, 0.002)                      \
    TUNE(qhp_threshold, -2096, -4096, -1024, 154, 0.002)          \
                                                                  \
    /* SEE PVS */                                                 \
    ...
    TUNE(lmr_noisy_div, 231, 98, 394, 15, 0.002)                  \
    TUNE(lmr_pv_node_red, 1266, 512, 2048, 77, 0.002)             \
    TUNE(lmr_alpha_raise_red, 528, 256, 1024, 38, 0.002)          \
    TUNE(lmr_not_improving_red, 548, 256, 1024, 38, 0.002)        \
    TUNE(lmr_in_check_red, 1040, 512, 2048, 77, 0.002)            \
    TUNE(lmr_cutnode_red, 1197, 512, 2048, 77, 0.002)             \
    TUNE(lmr_no_tt_red, 928, 512, 2048, 77, 0.002)                \
    TUNE(lmr_ttpv_red, 987, 512, 2048, 77, 0.002)                \
    ...
    TUNE(lmr_fut_red_base, 522, 250, 1000, 38, 0.002)             \
    TUNE(lmr_fut_red_mult, 103, 50, 200, 8, 0.002)                 \
    TUNE(lmr_fut_red, 677, 512, 2048, 77, 0.002)                  \
    ...
    TUNE(nodetm_avg_factor, 2336, 1024, 4096, 153, 0.002)         \
    TUNE(nodetm_frac_factor, 2192, 948, 3792, 142, 0.002)         \
    TUNE(d1plexity_base, 933, 394, 1576, 59, 0.002)               \
    TUNE(d1plexity_max_complexity, 213, 100, 400, 15, 0.002)      \
