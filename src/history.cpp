...
i32 History::get_quiet_stats(const Position& pos, Move move, i32 ply, Search::Stack* ss) const {
    ...
    i32  stats = m_main_hist[static_cast<usize>(pos.active_color())][move.from_to()][from_attacked * 2 + to_attacked];
...
void History::update_cont_hist(const Position& pos, Move move, i32 ply, Search::Stack* ss, i32 bonus) {
    ...
    if (ply >= 6 && (ss - 6)->cont_hist_entry != nullptr) {
        update_hist_entry_banger((*(ss - 6)->cont_hist_entry)[stm_idx][pt_idx][move.to().raw], conthist, bonus);
    }
}


void History::update_quiet_stats(const Position& pos, Move move, i32 ply, Search::Stack* ss, i32 bonus) {
    auto  to_attacked   = pos.is_square_attacked_by(move.to(), ~pos.active_color());
    auto  from_attacked = pos.is_square_attacked_by(move.from(), ~pos.active_color());
    usize stm_idx       = static_cast<usize>(pos.active_color());
...
void History::update_noisy_stats(const Position& pos, Move move, i32 bonus) {
    ...
    usize     pt_idx   = static_cast<usize>(pt) - static_cast<usize>(PieceType::Pawn);
    PieceType captured = move.is_en_passant() ? PieceType::Pawn : pos.piece_at(move.to());
    update_hist_entry(m_capt_hist[stm_idx][pt_idx][static_cast<usize>(captured)][move.to().raw], bonus);
}
...
void History::update_correction_history(const Position& pos, i32 depth, i32 diff) {
    ...
    usize white_non_pawn_index = static_cast<usize>(white_non_pawn_key % CORRECTION_HISTORY_ENTRY_NB);
    usize black_non_pawn_index = static_cast<usize>(black_non_pawn_key % CORRECTION_HISTORY_ENTRY_NB);
