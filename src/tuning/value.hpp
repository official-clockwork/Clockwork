...
namespace Clockwork {
    ...
    namespace Autograd {
        ...
        template <typename T = f64> class Value;
        template <typename T = f64> class Graph;
        ...
        public:
            ...
            T get_gradient() const { return m_gradient; }
            ...
            // Negation
            template <typename U> friend ValuePtr<T> operator-(ValuePtr<U> a);
            ...
            // Print << operator
            template <typename U> friend std::ostream& operator<<(std::ostream& os, const Value<U>& value);

        };

        template <typename T> Value<T>::Value(T data) : m_data(data) {};
        
        // TODO: might replace with std::make_shared for clearer semantics
        template <typename T> ValuePtr Value<T>::create(T data) {
        ...
        ValuePtr<T> Value<T>::exp(){
            ...
            result->m_backward_func = [this_value, result](){
                T grad = result->m_value * result->m_gradient; // Avoid recomputing std::exp, since grad of e^x is e^x, we can just use the one stored in the result var
        ...
        ValuePtr<T> Value<T>::sigmoid() {
            auto        this_value  = this->shared_from_this();
            ValuePtr<T> result      = Value<T>::create(
                1 / (1 + std::exp(-this_value->m_data))
            );
            result->m_dependencies  = {this_value};
            result->m_backward_func = [this_value, result]() {
                T grad = result->m_value * (1 - result->m_value); // Same trick as before, avoid recomputing sigmoid(x)
                this_value->m_gradient += grad;
            }; 
            return result;
        }
        ...
        ValuePtr<T> Value<T>::pow(ValuePtr<T> exponent){
            ...
            result->m_dependencies = {this_value};
            result->m_backward_func = [this_value, result](){
                T grad = exponent->m_value * std::pow(this_value->m_value, exponent->m_value - 1) * result->m_gradient;
                this_value->m_gradient += grad;
            };
            return result;
        }
        ...
        friend ValuePtr<T> operator-(ValuePtr<U> a){
            ValuePtr<T> result = Value<T>::create(static_cast<T>(-a->m_data));
            result->m_dependencies = {this_value};
            result->m_backward_func = [this_value, result]() {
        ...
        friend ValuePtr<T> operator+(ValuePtr<U> a, ValuePtr<U> b) {
            ValuePtr<T> result      = Value<T>::create(static_cast<T>(a->m_data) + static_cast<T>(b->m_data));
        ...
        template<typename U>
        template<typename T>
        friend ValuePtr<T> operator-(ValuePtr<U> a, ValuePtr<U> b) { // We are NOT cheaping out with a + (-b)
            ValuePtr<T> result = Value<T>::create(static_cast<T>(a->m_data) - static_cast<T>(b->m_data));
            result->m_dependencies  = {a, b};
            result->m_backward_func = [a, b, result]() {
                a->m_gradient += static_cast<U>(result->m_gradient);
        ...
        template<typename U>
        template<typename T>
        friend ValuePtr<T> operator*(ValuePtr<U> a, ValuePtr<U> b) { // We are NOT cheaping out with a * (std::pow(b,-1))
            ValuePtr<T> result = Value<T>::create(static_cast<T>(a->m_data) / static_cast<T>(b->m_data));
            result->m_dependencies  = {a, b};
            result->m_backward_func = [a, b, result]() {
