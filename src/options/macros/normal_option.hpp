...
struct Option : OptionBase {
    ...
    public:
        Option(const std::string& name, ParamType default_value, ParamType min_value, ParamType max_value)
            : OptionBase(name,
        ...
        Option(const std::string& name, ParamType default_value)
            : OptionBase(name,
              ...
              std::is_same_v<ParamType,std::string> ? OptionType::String :
                                                   OptionType::String ),
            value(default_value), default_value(default_value),
            min_value(ParamType()), max_value(ParamType()),
            has_range(false)
        {}
        ...
        [[nodiscard]] bool set_from_string(const std::string& str) override {
            ...
            if constexpr (std::is_same_v<ParamType,float>) {
                ...
                std::transform(str_copy.begin(), str_copy.end(), str_copy.begin(), [](unsigned char character){ return std::tolower(character); });
        ...
        void print_option_line() const override {
            std::cout << "option name " << get_option_name()
                      << " type "     << uci_type_name()
            ...
            if constexpr (std::is_same_v<ParamType,float>) {
                ...
                std::cout << " min " << output_min.str()
                          << " max " << output_max.str();
            }
            if constexpr (std::is_same_v<ParamType,int>) {
                std::ostringstream output_min, output_max;
                output_min << min_value; output_max << max_value;
                std::cout << " min " << output_min.str()
                          << " max " << output_max.str();
            }
            std::cout << "\n";
