...
struct PieceId {
    ...
    constexpr static PieceId king() { return {0}; }
...
public:
    ...
    [[nodiscard]] PieceId msb() const { return PieceId{static_cast<u8>(std::countl_zero(m_raw))}; }

    [[nodiscard]] PieceId lsb() const { return PieceId{static_cast<u8>(std::countr_zero(m_raw))}; }

    [[nodiscard]] u16 value() const { return m_raw; }

    [[nodiscard]] bool is_set(PieceId id) const { return (m_raw >> id.raw) & 1; }

    void clear(PieceId id) { m_raw &= ~id.to_piece_mask().m_raw; }

    void set(PieceId id) { m_raw |= id.to_piece_mask().m_raw; }
    ...
    public:
        ...
        PieceId operator*() const { return PieceId{static_cast<u8>(std::countr_zero(m_raw))}; }
    ...
    friend constexpr PieceMask operator~(PieceMask a) { return PieceMask{static_cast<u16>(~a.m_raw)}; }
    ...
    friend constexpr PieceMask& operator&=(PieceMask& a, PieceMask b) { return a = a & b; }
    friend constexpr PieceMask& operator|=(PieceMask& a, PieceMask b) { return a = a | b; }
...
struct Place {
    ...
    constexpr Place(Color color, PieceType pt, PieceId id) {
        raw =
          static_cast<u8>((static_cast<i32>(color) << COLOR_SHIFT) | (static_cast<i32>(pt) << PTYPE_SHIFT) | id.raw);
    }

    [[nodiscard]] constexpr bool      is_empty() const { return raw == 0; }
    [[nodiscard]] constexpr Color     color() const { return static_cast<Color>((raw & COLOR_MASK) != 0); }
    [[nodiscard]] constexpr PieceType ptype() const {
        return static_cast<PieceType>((raw & PTYPE_MASK) >> PTYPE_SHIFT);
    }
    [[nodiscard]] constexpr PieceId id() const { return PieceId{static_cast<u8>(raw & ID_MASK)}; }
...
struct Byteboard {
    ...
    [[nodiscard]] u8x64 to_vector() const { return std::bit_cast<u8x64>(mailbox); }

    [[nodiscard]] Bitboard get_empty_bitboard() const { return Bitboard{to_vector().zeros().to_bits()}; }

    [[nodiscard]] Bitboard get_occupied_bitboard() const { return Bitboard{to_vector().nonzeros().to_bits()}; }

    [[nodiscard]] Bitboard get_color_bitboard(Color color) const {
        u64  color_bb = static_cast<u64>(0) - static_cast<u64>(color);
        auto vec      = to_vector();
        return Bitboard{~vec.test(u8x64::splat(0x10)).to_bits() ^ color_bb} & get_occupied_bitboard();
    }
    ...
    [[nodiscard]] Bitboard bitboard_for(PieceType ptype) const {
        ...
        return Bitboard{(to_vector() & u8x64::splat(Place::PTYPE_MASK)).eq(u8x64::splat(p.raw)).to_bits()};
    }
...
struct Wordboard {
    ...
    [[nodiscard]] std::array<PieceMask, 64> to_mailbox() const { return std::bit_cast<std::array<PieceMask, 64>>(raw); }

    [[nodiscard]] Bitboard get_attacked_bitboard() const { return Bitboard{raw.nonzeros().to_bits()}; }
    ...
    friend inline Wordboard operator&(const Wordboard& a, const Wordboard& b) { return Wordboard{a.raw & b.raw}; }
